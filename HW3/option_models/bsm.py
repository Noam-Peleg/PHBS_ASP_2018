...
class BsmModel:
    ...
    def delta(self, strike, spot, texp, cp_sign=1):
        ...
        delta = np.log(forward/strike)/vol_std + 0.5*vol_std
        delta1 = ss.norm.cdf(delta)
    ...
    def vega(self, strike, spot, vol, texp, cp_sign=1):
        ...
        #defined in def delta
        delta = np.log(forward/strike)/vol_std + 0.5*vol_std
        
        vega1 = cp_sign*disc_fac*ss.norm.pdf(d1)*spot*np.sqrt(texp)
    ...
    def gamma(self, strike, spot, vol, texp, cp_sign=1):
        ...
        gamma1 = cp_sign*div_fac* ss.norm.pdf(d1) / (spot*vol_std)
