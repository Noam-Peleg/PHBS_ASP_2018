...
class ModelHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, spot, texp=None, cp_sign=1, setval=False, is_vol=True):
        ...
        '''
    #If prices are given (is_vol=False) convert the prices to vol first
    #def impvol --> vol = self.bsm_model.impvol(price, strike, spot, texp, cp_sign=cp_sign)
        ...
        def FUN(x):
            #step 1, as stated in class, need to define "fun" fx (sigma, alpha, rho, k_array, iv_given) and then transform the SAR variables. Rho is the sigmoid fx given in class. 
            # Follow this formula: bsm_vol(strike, forward, texp, _sigma, alpha=self.alpha, rho=self.rho) - vol
            sigma = np.exp(x[0])
            alpha = np.exp(x[1])
            rho = 2/(1+np.exp(-x[2]))-1
        ...
        #step 2 is to optimize the x_root fx for multidimensional root solving. sopt.root taken from Scipy website. Use optimization in same formulas as used above. 
        sol = sopt.root(FUN, np.array([0,0,0]))
        opt_root_x = sol.x
                       
        sigma = np.exp(opt_root_x[0])
        alpha = np.exp(opt_root_x[1])
        rho = 2/(1+np.exp(-opt_root_x[2]))-1
...
class ModelNormalHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, spot, texp=None, cp_sign=1, setval=False, is_vol=True):
        ...
        def FUN(x):
            #step 1, as stated in class, need to define "fun" fx (sigma, alpha, rho, k_array, iv_given) and then transform the SAR variables. Cannot use "exp" in normal model to transform so must change to "sqrt." Formula changes accordingly. 
            # Follow this formula: norm_vol(strike, forward, texp, _sigma, alpha=self.alpha, rho=self.rho) - vol
            sigma = np.sqrt(x[0]**2)
            alpha = np.sqrt(x[1]**2)
            rho = 2*x[2]/(1+x[2]**2)
        ...
        sigma = np.sqrt(opt_root_x[0]**2)
        alpha = np.sqrt(opt_root_x[1]**2)
        rho = 2*opt_root_x[2]/(1+opt_root_x[2]**2)
...
class ModelBsmMC:
    ...
    def bsm_vol(self, strike, spot, texp=None, sigma=None):
        ...
        use bsm_model
        '''
       #Definition of implied vol 
        texp = self.texp if(texp is None) else texp
        vol = self.bsm_model.impvol(price, strike, spot, texp, cp_sign=cp_sign)
        forward = spot * np.exp(texp*(self.intr - self.divr))
        #price from self.price()
        self.price(strike, spot, texp, sigma) = price
        
       
        #implied vol under bsm model
        iv_func = lambda _sigma: \
            bsm_vol(strike, forward, texp, _sigma, alpha=self.alpha, rho=self.rho) - vol
        sigma = sopt.brentq(iv_func, 0, 10)
        if(setval):
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1):
        ...
        '''
        #MC simulation slide formulas under SABR
        ...
        prices = np.mean(np.fmax(forward - strike[np.newaxis, :], 0)
        return prices
